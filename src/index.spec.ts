...
describe('Test vectors', () => {
  ...
  describe('Vector 1', () => {
    it('should have valid keys for vector 1 seed hex', () => {
      ...
      expect(bytesToHex(key)).toEqual(
        '2b4be7f19ee27bbf30c667b642d5f4aa69fd169872f8fc3059c08ebae2eb19e7'
      );
      expect(bytesToHex(chainCode)).toEqual(
        '90046a93de5380a72b5e45010748567d5ea02bbf6522f979e05c0d8d8ca9fffb'
      );
    });
    vector_1.forEach((vector) => {
      it(`should valid for ${vector.path}`, () => {
        const { key, chainCode } = derivePath(vector.path, vector_1_seed);
        expect({
          path: vector.path,
          key: bytesToHex(key),
  ...
  describe('Vector 2', () => {
    ...
    vector_2.forEach((vector) => {
      it(`should valid for ${vector.path}`, () => {
        const { key, chainCode } = derivePath(vector.path, vector_2_seed);
        expect({
          path: vector.path,
          key: bytesToHex(key),
          chainCode: bytesToHex(chainCode),
